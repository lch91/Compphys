...
Walker	MonteCarlo::getWalker(int    MCTrials, double walkProb, double time,
			      double timeStep, double    start, double  end){
	return Walker(MCTrials, walkProb, time, timeStep, start, end);
}
...
Matrix	Diffusion::solveJacobi2D(Matrix  init, double T,
			         double    dt, double h){
	int	N 	= init.getN();
	int	M 	= init.getM();
	// The boundry conditions assume N == M
	assert (N == M);
	double	alpha	= dt/(h*h);
	Matrix 	u	= Matrix(init);
	Matrix 	un	= Matrix(N,M);
	double** 	pu 	= u .getArrayPointer();
	double** 	pun 	= un.getArrayPointer();
	for (int j = 1; j*dt < T; j++) {
		// Print on certain time steps
		if(j%1000 == 0){
			cout << "computing - " << j << " iterations" << endl;
		}
		// Calculate next step in pun (pointer u next)
		pDiffusion::stepJacobi2D(pu,pun,alpha,N,M,T);
		// Copy values
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				pu[i][j] = pun[i][j];
...
Matrix	Diffusion::solveEuler2D(Matrix init, double T, 
				double   dt, double h){
	int	N 	= init.getN();
	int	M 	= init.getM();
	// The boundry conditions assume N == M
	...
	for (int j = 1; j*dt < T; j++) {
		...
		pDiffusion::stepEuler2D(pu,pun,alpha,N,M);
		// Copy values
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				pu[i][j] = pun[i][j];
		...
		for (int k = 0; k < N; k++) {
			// Here we assume x and y goes from 0 to 1
			double xy = float(k)/N;
			pu[0][k] 	= (1-xy)*exp(T);
			pu[N-1][k] 	= (1-xy)*exp(1+T);
			pu[k][0] 	= exp(xy+T);
			pu[k][N-1] 	= 0;
		}
	}
